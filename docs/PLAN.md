...
### あるべきシステムとビジネスとの関係性（ユーザーの考え）

今後このシステムを育て太らせていきながら学習をするわけだが以下の考えを参照しつつ、学習カリキュラムを考えてほしい。

システム設計（コード、サーバ、インフラなど）はたとえ要求や非機能要件がわりと厳密だったとしても（そもそも私はSIerみたいなやつじゃないのでそこは大概曖昧なプロジェクトにいる）将来の不確実性も含めて曖昧で踏み込んで考えると選択肢も多く迷い哲学的思想的な方向に傾きやすくなる感じがする。（それでも要求や非機能要件が厳密であるほどファクトベースで考えやすいか）。私は哲学や思想は好きだけどシステム開発というビジネスではそちらに傾きたくなくて（なぜならプライベートの哲学・思想は迷い思考することこそが醍醐味だがビジネスの現場ではそうではないから）標準であることを基礎にしてテストというガードレールに守られながらシステムを成長させていけることが重要なのではないかと考えている。昨今の当たり前であるAIの補助が入る場合も哲学的思想的に議論するより標準とファクトベースで議論をしたほうが建設的なはずだ。なぜならAIはある程度曖昧さはあろうがファクトベースで収斂する答えのある問いに対する応答のほうが有用と感じうる応答をしやすい、から（肌感）。システムにおいてビジネスを表現する、ではなく、ビジネスを勝負の土俵にしっかり乗せられるインフラを固くしよう、が私のスタンスだと思う。基本的にビジネスは不確実で流動的であるものだと体験してきている。だからビジネスをシステムで表現することにこだわりすぎないほうがいい。

ビジネスを表現しない、とまでは言っていない。
採用した標準形の範囲で結果として表現されることはあるが、
ビジネスを美しく・完全に表現することを目的にはしない。
なぜなら多くのビジネスは、不確実で、
曖昧さと融通を前提とする人間的関係の中で成立しているからだ。
だからシステムは、ビジネスを入力として受け取りつつ、
それをビジネスのまま安全に・速く・繰り返しデリバリーできる
インフラとしての能力を重視し、そこを固くする。
---
